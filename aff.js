...
puppeteer.launch({ headless: false, ignoreHTTPSErrors: true }).then(async browser => {
  let flag = false,
  jsonPath = path.resolve('affs.json')
...
let vote = async () => {
  ...
  if(process.argv[2] == 'g'){
    await granFratello.$eval('a[title="Giulia"]', el => el.click())
  ...
  if(msgOk.includes('Hai a disposizione ancora')){
    try{
      if(process.argv[2] == 'g'){
        console.log('Votei na Giulinha || Ainda há votos nessa sessão, revotando...\n')
      }else if(process.argv[2] == 'd'){
...
success = false
while(!flag){
  if(process.argv[2] == 'g' || process.argv[2] == 'd'){
